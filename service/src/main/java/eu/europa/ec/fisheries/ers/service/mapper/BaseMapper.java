    ...
    public class BaseMapper {
        ...
        public static List<AssetListCriteriaPair> mapMdrCodeListToAssetListCriteriaPairList(Set<AssetIdentifierDto> identifierDtoSet,List<String> vesselIdentifierSchemeList) {
            List<AssetListCriteriaPair> criteriaList = new ArrayList<>();
           //Iterator<String> listIterator=vesselIdentifierSchemeList.iterator();

            for (String keyFromMdr : vesselIdentifierSchemeList) {
                VesselIdentifierSchemeIdEnum identifierSchemeId=VesselIdentifierSchemeIdEnum.valueOf(keyFromMdr);
                //VesselIdentifierSchemeIdEnum identifierSchemeId = identifierDto.getIdentifierSchemeId(); // key from db hard coded

                if (null!=identifierSchemeId){
                    ConfigSearchField keyFromDto = VesselIdentifierMapper.INSTANCE.map(identifierSchemeId);
                   // String identifierId = identifierDto.getFaIdentifierId();
                    AssetListCriteriaPair criteriaPair = new AssetListCriteriaPair();
                    criteriaPair.setKey(keyFromDto);
                    criteriaPair.setValue(identifierSchemeId.value());
                    criteriaList.add(criteriaPair);
                }
            }
            //TODO : DELETE this b4 commiting
           /*
           for (AssetIdentifierDto identifierDto : identifierDtoSet) {
        ...
        public static Map<String,String> getVesselIdentifierSchemeMap(){
            Map<String,String> vesselIdentifierSchemeMap=new HashMap<String,String>();
            vesselIdentifierSchemeMap.put("CFR","cfrValue");
            vesselIdentifierSchemeMap.put("EXT_MARK","extValue");
            vesselIdentifierSchemeMap.put("IRCS","ircsValue");

            return vesselIdentifierSchemeMap;
        }
